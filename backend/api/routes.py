...
class RegisterRequest(BaseModel):
    ...
    crypto_salt: str = ""   # hex salt generated by client's CryptoEngine.generateSalt()
...
async def register(req: RegisterRequest):
    success = auth_manager.create_user(req.username, req.password, req.crypto_salt)
    if not success:
        raise HTTPException(status_code=400, detail="Registration failed. User may already exist or password too short.")
    # Return the salt so the client can derive the key immediately after registration
    salt = auth_manager.get_crypto_salt(req.username)
    return {"message": "User registered successfully", "username": req.username, "crypto_salt": salt}


@auth_router.get("/salt")
async def get_salt(username: str = Query(...)):
    """
    Return the user's crypto salt (public — not a secret).
    Client uses this + password to re-derive the AES-256-GCM key on every login.
    No auth required: salt is public knowledge (security comes from the password).
    """
    salt = auth_manager.get_crypto_salt(username)
    if not salt:
        raise HTTPException(status_code=404, detail="User not found")
    return {"username": username, "crypto_salt": salt}
